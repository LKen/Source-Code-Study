...
class Grid extends React.PureComponent<Props, State> {
  ...
  /**
   * TODO 这里 通过原生事件，记录 offsetTop offetLeft 触发业务绑定的方法，调用setState 更新 State 触发render
  ...
  invalidateCellSizeAfterRender({columnIndex, rowIndex}: CellPosition) {
    this._deferredInvalidateColumnIndex =
      typeof this._deferredInvalidateColumnIndex === 'number'
        ? Math.min(this._deferredInvalidateColumnIndex, columnIndex) // 重新渲染的列下标中最小值
        : columnIndex;
    this._deferredInvalidateRowIndex =
      typeof this._deferredInvalidateRowIndex === 'number'
        ? Math.min(this._deferredInvalidateRowIndex, rowIndex) // 重新渲染的行下标中最小值
        : rowIndex;
  }
  ...
  _calculateChildrenToRender(
    ...
    if (height > 0 && width > 0) {
      ...
      // Store for _invokeOnGridRenderedHelper() 获取各边界值
        ...
        if (!deferredMeasurementCache.hasFixedHeight()) {
          for (
            ...
            if (!deferredMeasurementCache.has(rowIndex, 0)) {
              columnStartIndex = 0;
              columnStopIndex = columnCount - 1; // 每一行总以cell 高度最大的那个看起，如果存在一个未被勘测的，那么就需要全部重新渲染
              break;
            }
          }
        }
