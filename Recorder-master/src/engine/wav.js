...
(function () {
  ...
  Recorder.prototype.wav = function (res, True, False) {
    ...
    writeString('RIFF');
    /* RIFF chunk length */
    write32(36 + dataLength); // 文件数据长度，从下一个字段首地址开始到文件末尾的总字节数。该字段的数值加 8 为当前文件的实际长度。头信息，一共 44字节
    /* RIFF type */
    writeString('WAVE');
    ...
    write16(bitRate / 8); // 采样帧大小。该数值为:声道数×位数/8。播放软件需要一次处理多个该值大小的字节数据,用该数值调整缓冲区
    /* bits per sample */
    write16(bitRate);
